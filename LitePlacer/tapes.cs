...
namespace LitePlacer
{
	class TapesClass
	{
        ...
        public bool GetPartHole_m(int TapeNum, int PartNum, out double ResultX, out double ResultY)
        {
            ...
            if (!MainForm.GoToFeatureLocationDownCam_m(0.2, out X, out Y, out double Atmp))
            {
                MainForm.ShowMessageBox(
        ...
        public bool GotoNextPartByMeasurement_m(int TapeNumber, out double HoleX, out double HoleY)
		{
            ...
            double ComponentA = 0;
            if (!MainForm.GoToFeatureLocationDownCam_m(0.5, out HoleX, out HoleY, out ComponentA))
			{
				MainForm.ShowMessageBox(
					"Can't find tape hole",
					"Tape error",
